...
Cron job "check-email" fires (every 30m, active 09:00-17:00)
    ...
    → Scheduler records execution status, then delivers via MessagingManager::broadcast_proactive(...)
    → Channel shuts down
...
Defined in `config.toml` under an agent. Seeded into the database on startup. Existing IDs are upserted, so config remains the source of truth for prompt, schedule, target, enabled/run-once, and timeout settings while preserving compatible persisted cursor state.
...
7. **Deliver** — If there is a delivery response, it is sent to the target via `MessagingManager::broadcast_proactive()`. Transient send failures retry with bounded backoff; permanent failures fail immediately. Unsupported proactive variants are treated as delivery failures, not silent skips.
